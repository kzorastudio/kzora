...
import { RotateCcw, Trash2, X, ArchiveRestore } from 'lucide-react'
        ...
        <div className="px-5 py-4 flex flex-col gap-3">
          ...
          {!isReservation && (
            <button
              ...
              disabled={loading}
              className="w-full flex items-start gap-3 p-3.5 rounded-xl border-2 border-primary/40 bg-primary/5 hover:bg-primary/10 transition-colors text-right disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
            >
              ...
              <div className="flex flex-col gap-0.5">
                ...
                <span className="text-xs font-arabic text-secondary leading-relaxed">
                  تحويل حالة الطلب إلى <strong>«ملغي مرتجع»</strong> وإعادة الكميات للمخزون مع <strong>بقاء بيانات الزبون وسجل الطلب كاملاً</strong>.
                </span>
              </div>
            </button>
          )}

          {/* Option 2 — Delete + Restore stock (Old Option) */}
          {!isReservation && (
            <button
              onClick={onDeleteAndRestore}
              disabled={loading}
              className="w-full flex items-start gap-3 p-3.5 rounded-xl border border-outline-variant/40 bg-surface-container hover:bg-surface-container-high transition-colors text-right disabled:opacity-50 disabled:cursor-not-allowed"
            >
              ...
              <div className="flex flex-col gap-0.5">
                ...
                <span className="text-xs font-arabic text-secondary leading-relaxed">
                  يتم حذف الطلب نهائياً من السجلات وإعادة الكميات للمخزون تلقائياً.
                </span>
              </div>
            </button>
          )}

          {/* Option 3 — Delete only */}
          <button
          ...
          >
            ...
            <div className="flex flex-col gap-0.5">
              <span className="text-sm font-arabic font-semibold text-error">
                حذف الطلب فقط
              </span>
              <span className="text-xs font-arabic text-secondary leading-relaxed">
                يتم مسح الطلب نهائياً من السجلات دون أي تأثير على المخزون.
              </span>
            </div>
          </button>
        </div>
